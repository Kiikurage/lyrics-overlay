...
`build-app.sh` は自己署名証明書「LyricsOverlay Dev」があればそれで署名し、無ければ ad-hoc に
落ちる。証明書は `scripts/create-signing-cert.sh` で作る(一度だけ)。

ad-hoc 署名には識別情報が無く、macOS はバイナリのハッシュでアプリを判別する。そのため
再ビルドや更新のたびに別のアプリとみなされ、オートメーションと音声録音の許可を取り直す
ことになる。証明書があれば「バンドル ID + 証明書」で判定されるので許可が維持される。
**Gatekeeper の警告は消えない**(公証には有償の Apple Developer Program が要る)。

アプリ内の更新(`UpdateChecker`)は、落としてきたアプリの署名者が動作中のものと一致することを
確かめてから入れ替える。したがって **証明書を作り直すと、それをまたぐ更新は失敗する**。
その回だけ手動での置き換えが必要になる。証明書を失っても影響はそれだけで、設定や表示位置は
バンドル ID に紐づくので保たれる。
